...
class ToShipWbAdapter (private val orders: MutableList<OrderFromWb>) :
    RecyclerView.Adapter<ToShipWbAdapter.OrderViewHolder>() {
    ...
    inner class OrderViewHolder(private val binding: ActivityOrderFromWbBinding) :
        ...
        fun bind(order: OrderFromWb) {
            ...
            binding.buttonTransfer.text="Отгрузка"
            binding.buttonCancel.setOnClickListener {
                onActionClickListener?.invoke(order)
            }
            binding.buttonTransfer.visibility = View.GONE
        }
    }
